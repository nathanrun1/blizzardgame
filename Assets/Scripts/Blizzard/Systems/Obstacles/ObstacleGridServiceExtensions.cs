...
namespace Blizzard.Obstacles
{
    ...
    public static class ObstacleGridServiceExtensions
    {
        ...
        {
            return obstacleGridService.Grids[obstacleLayer].Values
                .Where(o => (o.ObstacleFlags & obstacleFlags) == obstacleFlags);
        }

        public static IEnumerable<Vector2Int> GetAllObstacleLocationsWithFlags(
            this ObstacleGridService obstacleGridService, ObstacleFlags obstacleFlags,
            ObstacleLayer obstacleLayer = ObstacleConstants.MainObstacleLayer)
        {
            return obstacleGridService.Grids[obstacleLayer].ValidPositions.Where(p =>
                (obstacleGridService.Grids[obstacleLayer].GetAt(p).ObstacleFlags & obstacleFlags) == obstacleFlags);
        }

        /// <summary>
        /// Retrieves a random obstacle with matching obstacle flags on the main obstacle layer, or null if none exist
        /// </summary>
        public static Obstacle GetRandomObstacleWithFlags(this ObstacleGridService obstacleGridService,
            ObstacleFlags obstacleFlags)
        {
            if (_flagQueryCache.ContainsKey(obstacleFlags) && _flagQueryCache[obstacleFlags].Count > 3)
            {
                var obstacleIsValid =
                    obstacleGridService.Grids[ObstacleConstants.MainObstacleLayer].TryGetValue(
                        _flagQueryCache[obstacleFlags][_rand.Next(_flagQueryCache[obstacleFlags].Count)],
                        out var obstacle);
            ...
            _flagQueryCache[obstacleFlags] =
                obstacleGridService.GetAllObstacleLocationsWithFlags(obstacleFlags).ToList();

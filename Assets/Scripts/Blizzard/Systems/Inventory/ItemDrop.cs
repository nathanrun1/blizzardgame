...
namespace Blizzard.Environment
{
    public class ItemDrop : MonoBehaviour
    {
        ...
        /// </summary>
        [Header("Config")] [SerializeField] private float _randRotationRange;

        /// <summary>
        /// Enabled iff player can currently pick up this item drop
        /// </summary>
        [Header("Runtime Config")] [SerializeField]
        ...
        {
            if (!isActive || !collider.gameObject.CompareTag("Player"))
                return; // Ensure GameObject has "Player" tag and that drop is active
            Assert.That(_drop.item != null, "Test item data is not set!");

            var amountAdded = _inventoryService.TryAddItem(_drop.item, _drop.amount, true);
